...
public class AutumnXmlLanguageDriver extends XMLLanguageDriver {
    ...
    private boolean nodeDecorate(Configuration configuration, Node node) {
        ...
        if (Node.ELEMENT_NODE == nodeType || Node.ATTRIBUTE_NODE == nodeType) {
            if (AutumnHandlerRegistry.decorateSqlNode(configuration, node)) {
                return true;
            }
        }
        ...
        for (int i = 0, l = children.getLength(); i < l; i++) {
            ...
            switch (child.getNodeType()) {
                ...
                case Node.TEXT_NODE:
                    ...
                    String newContent = AutumnHandlerRegistry.evalSqlConfigFunction(configuration, content);
